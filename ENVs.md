...
| `FLA_TRITON_ASCEND`            | unset (`1`) | `0` / `1`  | Enable the triton-ascend (Huawei Ascend NPU) backend for gated delta-rule ops. Active when Triton reports backend `npu` and `torch.npu` is available. |
